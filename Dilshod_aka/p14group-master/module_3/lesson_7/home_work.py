...
request_list = ["posts" , "comments", "albums" , "users"]

con = sqlite3.connect("request.sqlite")
cur = con.cursor()
users_table = """
...
def get_data(request_name):
    response = httpx.get(f"https://jsonplaceholder.typicode.com/{request_name}")
    return response.json()

def save_data(table_name , data: list[dict]):
